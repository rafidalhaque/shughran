...
oTable3 = $('#PRData5').dataTable({
   ...
   // 'sAjaxSource': '<?= admin_url('branchorg/getListbranch'.($branch_id ? '/'.$branch_id : '').( $this->input->get('type') ? '?type='.$this->input->get('type') : '' ).( $this->input->get('year') ? '&year='.$this->input->get('year') : '' )  ) ?>',
   'sAjaxSource': '<?= admin_url('branchorg/getListbranch'.($branch_id ? '/'.$branch_id : '')  ) ?>',
...
<?php if ($Owner || $GP['bulk_actions']) {
    echo admin_form_open('branchorg/branchorg_actions'.($branch_id ? '/'.$branch_id : ''), 'id="action-form"');
} ?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= 'শাখা সংগঠন তালিকা   ' . ' (' . ($branch_id ? $branch->name : 'সকল শাখা') . ')'; ?>
        ...
        <div class="box-icon">
            ...
            <?php if($branch_id != null) {?>
              <li class="dropdown">
                    <a href="<?= admin_url('branchorg/addbranch/'.$branch_id.'/1') ?>">
                        <i class="icon fa fa-plus" data-placement="left" title="<?= lang("actions") ?>"><?= ' শাখা সংগঠন বৃদ্ধি করুন' ?></i>
                    </a>
                ...
                <li>	
                            <a href="<?= admin_url('branchorg/branchexport'.($branch_id ? '/'.$branch_id : '')) ?>" id="excel_export" data-action="export_excel">	
                ...
                <?php if (!empty($branches)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?= admin_url('branchorg/branchlist') ?>"><i class="fa fa-building-o"></i> <?= 'সকল শাখা' ?></a></li>
                            <li class="divider"></li>
                            <?php
                            foreach ($branches as $branch) {
                                echo '<li><a href="' . admin_url('branchorg/branchlist/' . $branch->id) . '"><i class="fa fa-building"></i>' . $branch->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
