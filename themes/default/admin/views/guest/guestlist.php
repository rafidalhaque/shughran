...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('guest/getGuest'.($branch_id ? '/'.$branch_id : '')) ?>',
...
<?php if ($Owner || $GP['bulk_actions']) {
    echo admin_form_open('guest/guest_actions'.($branch_id ? '/'.$branch_id : ''), 'id="action-form"');
} ?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= 'Guest list' . ' (' . ($branch_id ? $branch->name : 'সকল শাখা') . ')'; ?>
        </h2>
		...
		<div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <a class="tip" title="Add Guest" href="<?=  admin_url('guest/addguest'); ?>" data-toggle='modal' data-target='#myModal'>
                        <i class="icon fa fa-plus" data-placement="left" title="<?= lang("actions") ?>"> Add Guest</i>
                    </a>
