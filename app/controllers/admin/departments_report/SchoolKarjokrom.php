...
class SchoolKarjokrom extends MY_Controller
{
    ...
    function __construct()
    {
		...
		if(   $this->session->userdata('group_id')== 8 && $this->session->userdata('department_id')!=32) {
		 ...
		 if(  $this->session->userdata('group_id')== 8 && $this->session->userdata('department_id') ==32) {  //SchoolKarjokrom
        ...
        // Set the department id to 32
        $this->data['department_id'] = 32;
        ...
        // Conditions: The current year, the report type, branch ID, and department ID (32)
        $this->data['serial_info'] = $this->site->getOneRecord('serial_reports', '*', array('report_year' => date('Y'), 'report_type'=> $report_type['type'],'branch_id'=> $branch_id , 'dept_id'=>32), 'id desc', 1, 0);
